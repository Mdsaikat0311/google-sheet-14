...
/**
 * Checks if an order is excluded from Steadfast Entry according to user instructions:
 * "READY FOR DELIVERY TA J GOLOTE TARACING 9 DIGIT CODE AND TRAKING KONO STATUS NEI SEGOLO OI LIST A ADD HOBE"
 * An order is eligible for Ready for Delivery IF AND ONLY IF it has NO 9-digit tracking code in Column K and NO courier status in Column L.
 */
export const checkSteadfastEligibility = (order: Order) => {
  const tracking = String(order.trackingCode || '').trim();
  const has9DigitTracking = /^\d{9}$/.test(tracking);

  const courier = String(order.courierStatus || '').trim();
  const hasTrackingStatus = courier !== '';
  ...
  if (has9DigitTracking && hasTrackingStatus) {
    excludeReason = `K কলামে ৯ সংখ্যার ট্র্যাকিং কোড (${tracking}) ও L কলামে স্ট্যাটাস (${order.courierStatus})`;
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {order.trackingCode && /^\d{9}$/.test(order.trackingCode.trim()) && (
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-teal-950/70 text-[#7de3e0] border border-[#235863] shrink-0" title="৯ সংখ্যার ট্র্যাকিং কোড">
                        K: {order.trackingCode.trim()}
                      </span>
                    )}
                    {order.courierStatus && (
                      <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-purple-950/70 text-purple-300 border border-purple-800/40 shrink-0" title="কুরিয়ার স্ট্যাটাস">
                        L: {order.courierStatus.trim()}
                      </span>
                    )}
                    <button
                      type="button"
