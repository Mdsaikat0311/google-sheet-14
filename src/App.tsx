...
export default function App() {
  ...
  // 4b. Batch Send to Steadfast (Update Column M for multiple orders)
  const handleBatchSendToSteadfast = async (ordersToSend: Order[]): Promise<void> => {
    ...
    showToast(`${ordersToSend.length}টি অর্ডার স্টেডফাস্টে (কলাম M) পাঠানো শুরু হয়েছে...`);

    let successCount = 0;
    let failCount = 0;

    for (let i = 0; i < ordersToSend.length; i++) {
      const o = ordersToSend[i];
      const targetRow = resolveRowIndex(o);
      try {
        await updateSheetSteadfastAction(
          spreadsheetId,
          accessToken,
        ...
        console.error(`Error sending order ${o.id} to steadfast:`, err);
        failCount++;
      }
    }

    if (failCount === 0) {
      showToast(`✅ সফলভাবে ${successCount}টি অর্ডার স্টেডফাস্টে (কলাম M) পাঠানো হয়েছে!`, 'success');
    } else {
